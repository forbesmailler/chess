    ...
    while True:
        ...
        while not board.is_game_over(claim_draw=True):
            ...
            if board.turn == chess.BLACK:
                ...
                if white_root:
                    white_root.parent = None
            else:
                black_root = black_root.children.get(move)
                if black_root:
                    black_root.parent = None

        outcome = board.outcome(claim_draw=True)
        term = outcome.termination.name
        z = (
            1.0
            if (outcome.termination == chess.Termination.CHECKMATE and outcome.winner)
            else (-1.0 if outcome.termination == chess.Termination.CHECKMATE else 0.0)
        )
