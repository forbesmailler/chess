    ...
    def search(self, root_board: chess.Board) -> MCTSNode:
        ...
        for _ in range(self.sims):
            ...
            while not node.is_leaf():
                mv, node = max(
                    node.children.items(), key=lambda kv: uct(node, kv[1], self.c_puct)
                )
...
def train_on_batch(model, optimizer, batch):
    states = torch.stack([state_to_tensor(chess.Board(fen)) for fen, _, _ in batch]).to(
        DEVICE
    )
    pis = torch.stack([tensor_from_pi(pi) for _, pi, _ in batch]).to(DEVICE)
    zs = torch.tensor(
        [z for *_, z in batch], dtype=torch.float32, device=DEVICE
    ).unsqueeze(1)
    model.train()
    optimizer.zero_grad()
        ...
        for game_num in range(1, GAMES_PER_EPOCH + 1):
            ...
            while True:
                ...
                if move_count < TEMP_MOVES:
                    moves, weights = zip(
                        *[(mv, nd.N) for mv, nd in root.children.items()]
                    )
            ...
            logger.info(
                f"Game {game_num}/{GAMES_PER_EPOCH}: result={board.result()} ({end}), "
