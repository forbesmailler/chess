...
bool LichessClient::get_account_info(AccountInfo& info) {
    ...
    try {
        ...
        std::cout << "[DEBUG] Account: " << info.username << " (ID: " << info.id << ")" << std::endl;
...
bool LichessClient::test_connectivity() {
    ...
    if (res != CURLE_OK) {
        std::cout << "[DEBUG] Basic connectivity test failed: " << curl_easy_strerror(res) << std::endl;
        return false;
    }
    ...
    std::cout << "[DEBUG] Lichess connectivity OK (status: " << response.status_code << ")" << std::endl;
    return true;
}
...
LichessClient::HttpResponse LichessClient::make_request(const std::string& url, const std::string& method, 
    ...
    if (res == CURLE_OK) {
        ...
        std::cout << "[ERROR] CURL error for " << url << ": " << curl_easy_strerror(res) << std::endl;
...
void LichessClient::stream_lines(const std::string& url, std::function<void(const std::string&)> callback) {
    ...
    std::cout << "[DEBUG] CURL stream finished with result: " << curl_easy_strerror(res) << std::endl;
