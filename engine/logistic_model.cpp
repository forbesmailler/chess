...
bool LogisticModel::load_model(const std::string& model_path) {
    ...
    if (!file.is_open()) {
        ...
        if (!file.is_open()) {
            std::cerr << "Could not open model file: " << model_path << " or " << txt_path
                      << std::endl;
            std::cerr << "Please run export_model.py in the cpp/train/ directory to convert your "
                         ".joblib model to text format"
                      << std::endl;
    ...
    std::cout << "Loaded model with " << n_classes << " classes and " << n_features << " features"
              << std::endl;
...
std::vector<float> LogisticModel::predict_proba(const std::vector<float>& features) const {
    if (features.size() != static_cast<size_t>(n_features)) {
        std::cerr << "Feature size mismatch: expected " << n_features << ", got " << features.size()
                  << std::endl;
    ...
    for (int class_idx = 0; class_idx < n_classes; class_idx++) {
        ...
        for (int feature_idx = 0; feature_idx < n_features; feature_idx++) {
            logits[class_idx] +=
                weights[class_idx * n_features + feature_idx] * features[feature_idx];
        }
    }
